...
class RunningFragment : Fragment(), BluetoothDataListener {
    ...
    private fun startRunning() {
        ...
        Log.d("RunningDebug", "▶ run started, running=$running")
    }

    private fun stopRunning() {
        running = false
        Log.d("RunningDebug", "■ run stopped, running=$running")
        ...
        findNavController().navigate(
            R.id.action_runningFragment_to_runSummaryFragment, Bundle().apply {
                putDouble("distance", pet?.totalDistance ?: 0.0)
                putDouble("calories", pet?.totalCalories ?: 0.0)
                putLong  ("durationMin", (System.currentTimeMillis() - startTime) / 60000 )
            }
        )
    }
    ...
    override fun onBluetoothDataReceived(
        ...
        Log.d("RunningDebug", "recv($lat,$lon) running=$running")
        if (!running) return

        val pet = PetRepository.getCurrentPet() ?: return
        val map = mapFragment
        if (map == null || !map.isMapReady()) {
            Log.w("RunningFragment", "❌ Map 준비 전 – addLocation 생략")
            return
        }
        ...
        /* 거리/칼로리 계산 */
        val dist = if (lastLat != null && lastLon != null)
            haversine(lastLat!!, lastLon!!, lat, lon) else 0.0
        lastLat = lat; lastLon = lon

        val activityIdx = calculateActivityIndex(accX, accY, accZ)
        val calBurned   = calculateCalories(activityIdx,
            pet.weight.toString().toDoubleOrNull() ?: 10.0, dist)

        pet.totalDistance += dist / 1000.0
        pet.totalCalories += calBurned

        /* 목표 달성 여부 */
        val distGoal = goalDistance?.let { pet.totalDistance >= it } ?: false
        val calGoal  = goalCalories?.let { pet.totalCalories >= it } ?: false
        if (distGoal || calGoal) {
    ...
    private fun calculateActivityIndex(x: Float, y: Float, z: Float) =
        sqrt((x * x + y * y + z * z).toDouble())
    private fun calculateCalories(METidx: Double, wt: Double, dist: Double): Double {
        val MET   = if (METidx < 1.5) 2.0 else 6.0
        val hours = (dist / (METidx + 1)) / 3600.0
        return MET * wt * hours
    }
    ...
    fun captureRunSummaryAndSendToChat(layout: View, roomId: Int, userId: String) {
        ...
        ChatNetworkHelper.postJson("/uploadChat",json) { success ->
            if (success) Log.d("RunningShare", "러닝 결과 이미지 전송 성공")
        }
    }
