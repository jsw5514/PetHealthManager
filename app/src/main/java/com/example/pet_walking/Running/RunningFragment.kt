package com.example.pet_walking.Running.uploader
...
import com.example.pet_walking.Running.Map.MapFragment
import com.example.pet_walking.profile.data.PetProfile
...
import com.example.pet_walking_repectory.Running.uploader.StatsUploader
...
class RunningFragment : Fragment(), BluetoothDataListener {
    ...
    private fun stopRunning() {
        ...
        if (pet != null && userId != null) {
            Log.d("RunningDebug", "서버 업로드 시작: 거리 ${pet.totalDistance}, 칼로리 ${pet.totalCalories}")
    ...
    override fun onBluetoothDataReceived(lat: Double, lon: Double, accX: Float, accY: Float, accZ: Float) {
        ...
        val weight = pet.weight ?: 10.0
        val calories = StatusUtils.calculateCalories(activityIndex, weight, distance)

        Log.d("RunningDebug", "칼로리 계산: $calories kcal, 활동지수=$activityIndex")
